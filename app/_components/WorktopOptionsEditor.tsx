...
const WorktopOptionsEditor = (props: WorktopOptionsEditor) => {
  ...
  const handleNewWorktopOption = async () => {
    ...
    const { data, error } = await supabase.from('worktop_options').insert([
      { kitchen_type_id: props.kitchenType.id, worktop_id: addWorktop.id, price: priceInputValue }
    ]).select()
    if (error) console.log(error)
    if(data) setWorktopOptions([...worktopOptions, {
      id: data[0].id,
            ...
            <div className='flex flex-row gap-4 items-end'>
              <div className='flex flex-col gap-2'>
                <p className='text-lg font-semibold'>Worktop Make</p>
                  {worktopTypes && (
                    <select className='rounded py-2 px-4 text-text font-semibold text-lg bg-background' name="newWorktopOptionType" id="newWorktopOptionType" value={addWorktopTypeId} onChange={handleAddWorktopTypeId}>
                      {worktopTypes.map((worktopType: WorktopType) => (
                        <option value={worktopType.id} key={worktopType.id}>{worktopType.make}</option>
                      ))}
                    </select>
                  )}
              </div>
